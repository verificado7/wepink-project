...
module.exports = async (req, res) => {
  ...
  console.log('Dados recebidos para criar pagamento Pix:', { transaction_amount, description, payment_method_id, payer });

  if (!transaction_amount || !payment_method_id || !payer || !payer.email || !payer.identification || !payer.identification.number) {
    console.log('Parâmetros ausentes:', { transaction_amount, payment_method_id, payer });
  ...
  try {
    const accessToken = process.env.MP_ACCESS_TOKEN || 'APP_USR-3709341188085811-050219-1dde9cca4db5f13738f043119fdb7c30-220520212';
    ...
    if (!response.data.id) {
      console.error('ID do pagamento não encontrado na resposta do Mercado Pago');
    ...
    if (!response.data.point_of_interaction || !response.data.point_of_interaction.transaction_data || !response.data.point_of_interaction.transaction_data.qr_code) {
      console.error('QR Code não encontrado na resposta do Mercado Pago:', response.data);
